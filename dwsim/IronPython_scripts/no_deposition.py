# ----------------------------------------------------------------------------------
# Pasteurizer model — sensible + condensation (no deposition)
# ----------------------------------------------------------------------------------
...
# ---------------------------
# Check that mass flowrate of input stream is set to greater than max rate of condensation
rate_cond_max = h_coeff * Area_heat_transfer * (Temp_sat - Temp_init) / hfg_100
if (mass_flowrate_in <= rate_cond_max):
    print('Warning: Mass flowrate in needs to be adjusted upwards to be greater than rate of condensation')  # FIXED: Warning() → print()
...
for i in range(len(time)):
    if i == 0:
        ...
        if Ts_prev > Temp_sat:
            ...
            # CONDENSATION PHASE (no deposition)
            ...
            numerator = mass_flowrate_in * (enthalpy_in - enthalpy_out) + (rc - mass_flowrate_in * (1.0-quality_inst[-1])) *hfg_100

            T_b_new = Tb_prev + time_step * numerator / (m_bagasse_init * cp_bagasse)
            Temp_bagasse.append(T_b_new)

    # update mass of bagasse
    m_bagasse.append(m_bagasse[i-1])

    # update inst quality of steam
    if i == 0:
       qi = qi_init
    else:
        qi_prev = quality_inst[-1]
        qi = qi_prev + time_step * ((mass_flowrate_in * (1 - qi_prev) - rate_cond[i-1]) / m_steam)
    # time_step should be less than 2*m_steam / mass_flowrate_in
    # Clamp qi between 0 and 1 for physical consistency
    qi = max(0.0, min(1.0, qi))
    quality_inst.append(qi)
    
    # update mass of condensed steam
    ...
    # early stop if bagasse target achieved
